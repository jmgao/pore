...
impl<'repo> BranchInfo<'repo> {
  ...
  fn from_ref_name(repo: &'repo git2::Repository, ref_name: &str) -> Result<BranchInfo<'repo>, Error> {
    BranchInfo::from_ref(
      repo
        .find_reference(ref_name)
        .context(format!("could not find reference {}", ref_name))?,
    )
  }
}
...
fn summarize_upload(
  ...
  lines.push(format!(
    ...
    BRANCH_STYLE.apply_to(&dest.name),
    if autosubmit {
...
impl Tree {
  ...
  pub fn upload(
    ...
    for project in projects {
      ...
      let dest_branch_info =
        BranchInfo::from_ref_name(&repo, format!("{}/{}", remote_name, dest_branch_name).as_str())?;
